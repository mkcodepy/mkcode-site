...
export const Route = createFileRoute("/pt/projetos/$slug")({
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.proj.title.pt} — MK CODE` },
          { name: "description", content: loaderData.proj.description.pt },
          { property: "og:title", content: `${loaderData.proj.title.pt} — MK CODE` },
          { property: "og:description", content: loaderData.proj.description.pt },
          { property: "og:type", content: "article" },
        ]
      : [{ title: "Projeto — MK CODE" }, { name: "robots", content: "noindex" }],
  }),
  component: ProjectDetailPT,
  notFoundComponent: NotFoundPT,
});

function ProjectDetailPT() {
    ...
    <>
      <section className="pt-40">
        <Container>
          ...
          <p className="mt-6 max-w-2xl text-ink-2">{p.description.pt}</p>
        </Container>
      </section>
...
function NotFoundPT() {
  return (
    <section className="pt-40">
      <Container>
        <Eyebrow>404</Eyebrow>
        <h1 className="mt-4 font-display text-3xl">Rota não encontrada.</h1>
        <Link to="/pt/projetos" className="mt-6 inline-block font-mono text-[11px] tracking-[0.22em] text-cyan">
          ← PROJETOS
        </Link>
      </Container>
    </section>
